...
app.get("/api/posts", async (req, res) => {
  try {
    ...
    const userPosts = new Set(
      userPostRows.rows.map(r => String(r.post_id))
    );

    const commentCounts = new Map(
      commentRows.rows.map(r => [
        String(r.comment_id),
        Number(r.likes || 0)
      ])
    );

    const userComments = new Set(
      userCommentRows.rows.map(r => String(r.comment_id))
    );
...
async function createAccountNotification(
  ...
  await db.query(
    `INSERT INTO notifications
     (id, recipient_id, actor_id, type, title, text, target_id)
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.query(
        ...
        FROM (
          SELECT
            ...
            COALESCE(u.avatar,'') AS other_avatar,
      ...
      console.error(
        "ERRO /api/messages/conversations:",
        error
      );
